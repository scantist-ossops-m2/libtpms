...
elliptive curve cryptography (ECC) curves. Those verbs enumerated under
...
(Baretto-Naehrig) curves (ecc-bn-p256, ecc-bn-638). If omitted then the
...
Minimum key sizes for rsa, ecc, tdes, aes, and camellia can be described.

When new algorithms and ECC curves are added to the TPM , then this JSON
object will return more algorithms in the future.
