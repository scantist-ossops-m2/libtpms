...
The FIPS profile disables several algorithms that are not FIPS-approved,
such as SHA-1, Triple DES, Camellia and RSA-1024 keys.
Neither one of these profiles allows any modifications by the user. The
'custom' profile is the only one that a user may modify and disable any
commands and/or algorithms that may be disabled.
...
Note that the I<custom> profile is currently the only profile that allows
customizations of enabled Algorthms and Commands.
...
disabled commands. Determining the list of algorithms and commands to
enable/disable is left to the user.
...
versions exist and that may be a target for migration, then the oldest
libtpms version's supported profile should be chosen. If libtpms v0.9
exists then this should be the 'null' profile, for libtpms v0.10 this may
...
By adding this verb to the I<Attributes> a TPM 2 can be run on a
FIPS-enabled host. Note that this does not provide a FIPS-140-2/3
compliant TPM 2.
