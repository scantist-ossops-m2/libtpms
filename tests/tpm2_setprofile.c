...
static const struct {
    ...
    {
        .profile = NULL, // exp_profile will need to be adjusted
        .exp_fail = false,
        .exp_profile =
          "{\"ActiveProfile\":{"
            "\"Name\":\"default-v1\","
        ...
        .exp_profile =
          "{\"ActiveProfile\":{"
            ...
            "\"Algorithms\":\"rsa,rsa-min-size=1024,hmac,aes,aes-min-size=128,mgf1,"
                             ...
                             "kdf2,kdf1-sp800-108,ecc,ecc-nist,symcipher,cmac,ctr,"
                             "ofb,cbc,cfb,ecb,ecc-min-size=256\","
            "\"Attributes\":\"fips-host,fips-140-2\","
            "\"Description\":\"This profile allows the TPM 2 to run on a FIPS-enabled "
...
int main(void)
{
    ...
    for (i = 0;
        ...
        if (testcases[i].exp_fail && !res) {
            fprintf(stderr, "TPMLIB_SetProfile() did not fail as expected for testcase %d.\n", i + 1);
            goto exit;
        }
        if (res) {
            if (testcases[i].exp_fail) {
                printf("Passed test case %d (failed as expected)\n", i + 1);
                continue;
            }
        ...
        for (j = 0;
            ...
            if (ntohl(len) != rlength || memcmp(rbuffer, testcases[i].tx[j].rsp, rlength)) {
                fprintf(stderr, "Response to command %u is different than expected.\n",
                        j + 1);
                for (n = 0; n < rlength; n++)
            ...
            printf("  Received expected response to command %u\n", j + 1);
        }
